...
function DeltaBase(options, callback) {
  ...
  this.index = {};
  // A list of keys, some might not have been loaded
  this.keys = {};
...
DeltaBase.prototype._loadKeys = function(callback) {
  if (this._keysLoaded) {
    return callback();
  }
  var db = this;
  fs.readdir(this.paths.docs, function(err, files){
    if (err) return callback(err);
    // TODO: Use the keys ist to opimise away some other file operations
    for (i in files) {
      if (!_.hasOwn(db.keys, files[i])) {
        db.keys[files[i]] = true;
      }
    }
    db._keysLoaded = true;
    callback();
 ...
 */
DeltaBase.prototype.list = function(callback) {

  var db = this;
  // Check all our keys are loaded
  this._loadKeys(function(){
    async.map(_.keys(db.keys), function(key, cb) {
      db.get(key, cb);
