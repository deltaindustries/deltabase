...
function DeltaBase(options, callback) {
  ...
  function onReady() {
    ...
    if (this._readyError) {
      return callback(this._readyError)
    }

    // Process the queue
    this._ready = true;
    async.series(db._queued, function(err,result){
      // NOT passing any errors since these should have been originally
      // by the caller
      callback(null, db);
    });
  }
 ...
 * @param {Function} callback
 */
DeltaBase.prototype.set = function(id, doc, callback) {
  var $meta, text;
  if (!_.isString(id)) {
...
/** Update a set of fields on a document in the store and trigger areindex */
